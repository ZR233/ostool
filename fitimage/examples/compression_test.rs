...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "Compressed FIT image size: {} bytes",
        fit_data_compressed.len()
    );
    ...
    println!(
        "Uncompressed FIT image size: {} bytes",
        fit_data_uncompressed.len()
    );
    ...
    if fit_data_compressed.len() < fit_data_uncompressed.len() {
        ...
        println!(
            "✅ Compression saved {} bytes ({}% reduction)",
            savings,
    ...
    for (name, data) in [
        ("compressed", &fit_data_compressed),
        ("uncompressed", &fit_data_uncompressed),
    ] {
